...
function mapTx(row) {
  return { id: row.id, assetId: row.asset_id, value: Number(row.value), date: row.date, note: row.note, createdAt: row.created_at };
}
...
export async function addTransaction(tx) {
  const { data, error } = await supabase
    .from('transactions')
    .insert({ asset_id: tx.assetId, user_id: _userId, value: tx.value, date: tx.date, note: tx.note || null })
    .select()
    .single();
  if (error) throw error;
  ...
  if (data.assets && data.assets.length > 0) {
    ...
    if (data.transactions && data.transactions.length > 0) {
      const rows = data.transactions
        ...
        .map((tx) => ({ asset_id: idMap[tx.assetId], user_id: _userId, value: tx.value, date: tx.date, note: tx.note || null }));
