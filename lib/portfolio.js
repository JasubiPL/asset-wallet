...
export function getAssetPerformance(assetId) {
  const txs = Storage.getTransactionsByAsset(assetId);
  if (txs.length < 1) {
    return { currentValue: 0, initialValue: 0, change: 0, changePercent: 0, history: [] };
  }
  ...
  return {
    currentValue, initialValue, change, changePercent,
    history: txs.map((t) => ({ date: t.date, value: t.value, note: t.note, id: t.id })),
  };
}
...
export function getPortfolioSummary() {
  ...
  const totalChangePercent = totalInitialValue !== 0 ? (totalChange / totalInitialValue) * 100 : 0;

  return { totalCurrentValue, totalInitialValue, totalChange, totalChangePercent, assetCount: assets.length, assets: assetDetails };
}
